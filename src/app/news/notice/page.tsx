...
export const metadata: Metadata = {
  ...
  openGraph: { title: TITLE, description: DESCRIPTION, url: `${process.env.NEXT_PUBLIC_SITE_URL}/news/notice` },
...
export default async function NewsNoticePage({ searchParams }: Props) {
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-end gap-3 mb-5">
          {user && (
            <Link href="/news/notice/new" className="shrink-0 hidden sm:flex items-center gap-1.5 px-4 py-2 bg-sky-600 text-white text-sm font-semibold rounded-lg hover:bg-sky-700 transition-colors">
              <PenSquare size={14} />
              글쓰기
            </Link>
          )}
        ...
        <div className="flex flex-col sm:block gap-2 mb-6">
          ...
          {user && (
            <Link href="/news/notice/new" className="sm:hidden flex items-center justify-center gap-1.5 py-2.5 bg-sky-600 text-white text-sm font-semibold rounded-lg hover:bg-sky-700 transition-colors">
              <PenSquare size={14} />
              글쓰기
            </Link>
          )}
        ...
        <div className="border-t border-slate-200 pt-1">
          {q ? (
            ...
            <>
              ...
              {totalCount === 0 && (
                <div className="py-16 text-center text-slate-400 text-sm">아직 게시글이 없습니다.</div>
              )}
            </>
          )}
        </div>

        {!q && <Pagination currentPage={page} totalPages={totalPages} basePath="/news/notice" />}
      </div>
    </>
  )
}
...
function PostRow({ post, isPinned, rowNumber }: PostRowProps) {
  ...
  return (
    <Link href={`/news/notice/${post.id}`} className={cn('group py-3.5 px-2 -mx-2 rounded-lg hover:bg-slate-50 transition-colors flex sm:hidden items-center gap-2', isPinned && 'bg-slate-50/80 hover:bg-slate-100/80')}>
      {isPinned ? (
...
function PostRowDesktop({ post, isPinned, rowNumber }: PostRowProps) {
  ...
  const isNew = isNewPost(post.created_at)
  return (
    <Link href={`/news/notice/${post.id}`} className={cn('group hidden sm:flex items-center gap-3 py-3.5 px-2 -mx-2 rounded-lg hover:bg-slate-50 transition-colors', isPinned && 'bg-slate-50/80 hover:bg-slate-100/80')}>
      {isPinned ? (
        <span className="shrink-0 text-xs px-2 py-0.5 rounded-md font-medium bg-red-50 text-red-600 ring-1 ring-inset ring-red-200">공지</span>
      ) : (
        <span className="shrink-0 text-xs w-8 text-center font-medium text-slate-400">{rowNumber}</span>
...
function PostRowBoth(props: PostRowProps) {
  return <><PostRow {...props} /><PostRowDesktop {...props} /></>
}
