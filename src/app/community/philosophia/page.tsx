// 최더함의 철학시가 목록 페이지 — 다크 포레스트 에디토리얼 톤의 영상 아카이브. 현재는 디자인 검토용 목업 데이터로 렌더한다
...
import { MOCK_VIDEOS } from '@/features/philosophia/mock-videos'
...
// Tailwind는 동적으로 조합한 클래스명을 스캔하지 못하므로 지연값을 정적 문자열로 나열한다
const STAGGER = [
  '[animation-delay:0ms]',
...
export default async function PhilosophiaPage({ searchParams }: Props) {
  ...
  const videos = category === '전체' ? MOCK_VIDEOS : MOCK_VIDEOS.filter((v) => v.category === category)
        ...
        <div className="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
          ...
          <div className="mb-8 flex flex-wrap items-center gap-x-6 gap-y-3">
            {CATEGORIES.map((cat) => (
              <Link
                key={cat}
                href={cat === '전체' ? '/community/philosophia' : `/community/philosophia?category=${cat}`}
            ...
            >
              {String(videos.length).padStart(3, '0')} ENTRIES
            </span>

            {/* 목업 단계 — 로그인·권한 체크와 대상 라우트는 DB 연동 시 추가한다 */}
            <Link
              href="/community/philosophia/new"
          ...
          {videos.length === 0 ? (
            ...
            <div className="grid grid-cols-1 border-t border-l border-[#EDE7D6]/15 sm:grid-cols-2 xl:grid-cols-4">
              {videos.map((video, i) => (
                <Link
                  key={video.id}
                  href={`/community/philosophia/${video.id}`}
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`${plexMono.className} pt-2 text-[13px] tracking-[0.12em] text-[#D9B441]`}>
                      {video.series}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-1 flex-col justify-between gap-6">
                    <h2
                      className={`${batang.className} text-lg leading-snug text-[#EDE7D6] transition-colors duration-300 group-hover:text-[#D9B441]`}
                    >
                      {video.title}
                    </h2>
                    <div
