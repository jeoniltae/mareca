...
export default async function CommunityVoicePage({ searchParams }: Props) {
    ...
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-end gap-3 mb-5">
          {user && (
            <Link href="/community/voice/new" className="shrink-0 hidden sm:flex items-center gap-1.5 px-4 py-2 bg-sky-600 text-white text-sm font-semibold rounded-lg hover:bg-sky-700 transition-colors">
              <PenSquare size={14} />
              글쓰기
            </Link>
          )}
        ...
        <div className="flex flex-col sm:block gap-2 mb-6">
          ...
          {user && (
            <Link href="/community/voice/new" className="sm:hidden flex items-center justify-center gap-1.5 py-2.5 bg-sky-600 text-white text-sm font-semibold rounded-lg hover:bg-sky-700 transition-colors">
              <PenSquare size={14} />
              글쓰기
            </Link>
          )}
        ...
        {!q && <Pagination currentPage={page} totalPages={totalPages} basePath="/community/voice" />}
      </div>
    </>
  );
}
...
function PostRow({ post, isPinned, rowNumber }: PostRowProps) {
  ...
  return (
    <Link href={`/community/voice/${post.id}`} className={cn("group py-3.5 px-2 -mx-2 rounded-lg hover:bg-slate-50 transition-colors flex sm:hidden items-center gap-2", isPinned && "bg-slate-50/80 hover:bg-slate-100/80")}>
      {isPinned ? (
...
function PostRowDesktop({ post, isPinned, rowNumber }: PostRowProps) {
  ...
  const isNew = isNewPost(post.created_at);
  return (
    <Link href={`/community/voice/${post.id}`} className={cn("group hidden sm:flex items-center gap-3 py-3.5 px-2 -mx-2 rounded-lg hover:bg-slate-50 transition-colors", isPinned && "bg-slate-50/80 hover:bg-slate-100/80")}>
      {isPinned ? (
        <span className="shrink-0 text-xs px-2 py-0.5 rounded-md font-medium bg-red-50 text-red-600 ring-1 ring-inset ring-red-200">공지</span>
      ) : (
        <span className="shrink-0 text-xs w-8 text-center font-medium text-slate-400">{rowNumber}</span>
