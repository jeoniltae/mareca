...
### [버그 수정] 죽은 영상 검증의 경쟁 상태 (2026-08-29 코드리뷰)

- **문제** — 앞서 넣은 죽은 영상 검증이 우회 가능한 상태였다.
  `onChange`가 `thumbnailMissing`을 즉시 `false`로 되돌리는데 실제 판정은 미리보기 이미지의 `onLoad`에서
  비동기로 일어난다. **썸네일이 로드되기 전에 등록을 누르면 가드를 그냥 통과**했다.
  수정 모드도 초기값이 `false`라 마운트 직후 제출하면 검사를 거치지 않았다
- 조작이 아니라 **평범하게 빠르게 클릭하면 재현**되는 문제라, 기능을 넣은 목적 자체가 무력화된 상태였다
- **수정** — `features/youtube/verify-thumbnail.ts` 신규.
  제출 시점에 `startTransition` 안에서 **한 번 더 확인**한다. 기존 `thumbnailMissing` state에 의존하지 않는
  독립적인 검사라 state가 낡아 있어도 새지 않는다. `PhilosophiaForm` · `ReformedTVForm` 양쪽 적용
  - **판정하지 못하면 통과시킨다**(타임아웃 4초 / 이미지 에러 시 `false`).
    멀쩡한 영상의 등록을 막는 오탐이, 죽은 영상 하나를 통과시키는 미탐보다 나쁘다는 기존 판단과 같은 방향
  - 검사는 `startTransition` 안에 두어 진행 중 버튼이 `저장 중...`으로 바뀐다
- **검증 · 그리고 검증하지 못한 것**
  - 임시 라우트로 실제 폼을 띄워 확인 후 **라우트 삭제**(참조 0건 확인). 빌드·lint 통과
  - 죽은 ID → 경고 노출 + **제출 차단**(URL 유지) / 실재 ID → **통과**(저장 시도로 진행, 오탐 없음)
  - **경쟁 구간 자체는 재현하지 못했다.** React가 입력을 반영하는 데 80ms 이상 걸리고
    썸네일은 250ms 안에 로드돼 캐시되는데, 자동화로 그 사이를 겨냥할 해상도가 안 나온다.
    제출 시점 검사도 캐시 히트라 네트워크 요청이 늘지 않아 실행 여부를 밖에서 관측할 수 없었다
  - 즉 **이 수정의 근거는 코드 논리**(가드가 더 이상 낡은 state에 의존하지 않음)이고,
    실측으로는 "죽은 영상 차단 / 정상 영상 통과"라는 양쪽 경로가 깨지지 않았다는 것까지만 확인했다
  - 조사 중 "즉시 제출 시 차단됨" 같은 중간 결과가 몇 번 나왔으나 **전부 자동화 아티팩트였다**
    (`fill` 직후 클릭하면 React 반영 전이라 폼이 제출조차 되지 않는다 — `validity.valid: false`).
    같은 방식으로 이 부분을 검증하려는 다음 사람은 이 함정을 먼저 알고 시작할 것
