...
- **[미착수] 게시판 추가 — 최더함의 철학시가 (2차: DB 연동 · 상세/등록/수정 · SEO)**
  ...
  **수동 검증 — 로그인이 필요해 사용자가 직접 수행한다. `npm run dev` 후 진행**
  - [ ] (1) 등록 — `/community/philosophia/new`에서 유튜브 URL·제목·카테고리 입력 후 등록 → **상세로 이동**하는지
    - 잘못된 URL(`https://example.com`) 입력 시 `올바른 유튜브 URL을 입력해주세요.` 노출되는지
    - URL 입력 시 **썸네일 미리보기**가 뜨는지
  - [ ] (2) 목록 — 등록한 글이 보이고 **실제 유튜브 썸네일**이 렌더되는지, 스크림 때문에 재생 아이콘이 묻히지 않는지
  - [ ] (3) 상세 — **compact 히어로** · 플레이어 · 설명 · 공유 레이아웃.
    특히 **`ShareButtons`의 '링크 복사' 버튼이 다크 배경에서 읽히는지**(하위 선택자로 덮어쓴 부분)
  - [ ] (4) 조회수 — 상세 진입 시 1 증가하고, 목록으로 돌아가면 반영되는지
  - [ ] (5) 수정 → 반영, 삭제 → 목록 복귀
  - [ ] (6) 권한 — 타인 글에 수정·삭제 버튼이 안 보이는지 / 관리자 계정은 보이는지
  - [ ] (7) **13건 이상 등록** 후 — 페이지네이션 2페이지 동작,
    **2페이지 넘버링이 `01`이 아니라 `13`으로 이어지는지**, 카테고리 필터와 `page` 파라미터 조합
  - [ ] (8) 숏츠(세로 영상) 썸네일이 16:9 카드에서 깨지지 않는지
  - [ ] (9) 긴 제목에서 `line-clamp-2`가 의도대로 잘려 카드 높이가 균일한지
  - [ ] (10) 스켈레톤 — DevTools 네트워크 스로틀링(Slow 3G)으로 목록·상세 진입 시 **다크 스켈레톤이 뜨고
    실제 콘텐츠로 바뀔 때 흔들림이 없는지**
  - [ ] (11) 다크(목록·상세) ↔ 라이트(등록·수정) 전환이 어색하지 않은지
  - [ ] (12) `/sitemap.xml`에 **게시글 상세 URL이 자동 포함**되는지 (`BOARD_PATH_MAP` 동작 확인)
  - [ ] (13) 375 / 768 / 1440px
